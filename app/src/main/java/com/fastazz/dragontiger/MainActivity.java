...
public class MainActivity extends Activity {
    ...
    void render() {
        ...
        for(int i=start;i<rounds.size();i++){ TextView t=new TextView(this); t.setText(rounds.get(i).r); t.setTextSize(20); t.setPadding(12,8,12,8); patternRow.addView(t); pat.append(rounds.get(i).r.equals("D")?"D":rounds.get(i).r.equals("T")?"T":"S"); }
        String p=pat.length()>=3?pat.substring(pat.length()-3):pat.toString(); patternText.setText("Current 3-pattern: "+p);
        int d=0,t=0,same=0; for(Round r:rounds){if(r.r.equals("D"))d++;else if(r.r.equals("T"))t++;else same++;}
        summary.setText("D: "+d+" ("+pct(d,rounds.size())+")   T: "+t+" ("+pct(t,rounds.size())+")   SAME: "+same+" ("+pct(same,rounds.size())+")");
        int changes=0; for(int i=1;i<rounds.size();i++) if(!rounds.get(i).r.equals(rounds.get(i-1).r)) changes++; summary.append("\nResult changes: "+changes);
        if(p.length()==3){int aD=0,aT=0,aS=0,n=0; for(int i=0;i+3<rounds.size();i++){String q="";for(int j=i;j<i+3;j++)q+=rounds.get(j).r.equals("D")?"D":rounds.get(j).r.equals("T")?"T":"S"; if(q.equals(p)){n++;String r=rounds.get(i+3).r;if(r.equals("D"))aD++;else if(r.equals("T"))aT++;else aS++;}} patternAnalysis.setText("Current pattern: "+p+"\nOccurrences with next result: "+n+"\nD: "+aD+" ("+pct(aD,n)+")   T: "+aT+" ("+pct(aT,n)+")   SAME: "+aS+" ("+pct(aS,n)+")");} else patternAnalysis.setText("Not enough data for 3-result pattern analysis.");
        numberAnalysis.setText("Historical analysis is based on the loaded rounds.");
    }
}
