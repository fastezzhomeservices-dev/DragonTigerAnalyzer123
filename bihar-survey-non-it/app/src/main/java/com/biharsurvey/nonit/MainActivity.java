...
public class MainActivity extends AppCompatActivity {
    ...
    void buildHeaders(){
        ...
        // Old BIHAR FAR & EAM survey modules: keep the original category workflow and make these modules open on Android.
        HEADERS.put("Vehicle",new String[]{"Unique Id","ENTRYDATE","Surveyor","Hierarchy","Discom/ Organization Name","Zone Name","Circle Name","Division Name","Subdivision Name","Section Name","State","District Name","Block Name","Gram Panchyat Name","Village Name","Pincode","Office Name","Vehicle ID","TYPE_OF_VEHICLE","VEHICLE DESCRIPTION","VEHICLE NO","Manufacturer Name","Model Name","Manufacturing Date","Purchase Order Number","Purchase Order Date","In-operational Date","Condition","Status","Remarks","Address","Photo"});
        HEADERS.put("Plant & Machinery",new String[]{"Unique Id","ENTRYDATE","Surveyor","Hierarchy","Discom/ Organization Name","Zone Name","Circle Name","Division Name","Subdivision Name","Section Name","State","District Name","Block Name","Gram Panchyat Name","Village Name","Pincode","Office Name","Plant Type","Machinery Type","Equipment Number","Equipment Name","Manufacturer Name","Model Name","Serial Number","Manufacturing Date","Purchase Order Number","Purchase Order Date","Commissioning Date","In-operational Date","Working Usable","Condition","Asset Lifecycle Status","Warranty Date","Warranty Expiry Date","Status","Remarks","Address","Photo"});
        HEADERS.put("Store Inventory",new String[]{"Unique Id","ENTRYDATE","Surveyor","Hierarchy","Discom/ Organization Name","Zone Name","Circle Name","Division Name","Subdivision Name","Section Name","State","District Name","Block Name","Gram Panchyat Name","Village Name","Pincode","Store Name","Item Type","Item Name","Item Code","Serial Number","Quantity","Unit","Manufacturer Name","Model Name","Purchase Order Number","Purchase Order Date","Warranty Date","Condition","Working Usable","Status","Remarks","Address","Photo"});
    }
    ...
    void home(){
        ...
        TextView menu=label("☰"); menu.setTextColor(android.graphics.Color.WHITE); menu.setTextSize(28); menu.setGravity(Gravity.CENTER); header.addView(menu,new LinearLayout.LayoutParams(dp(48),dp(58))); menu.setOnClickListener(v->showSidebar());
...
User: "+currentUser());app.setTextColor(primaryText());panel.addView(app);
    ...
    void applyThemeSelection(int t){getPreferences(0).edit().putInt(PREF_THEME,t).apply();}
    ...
    int primaryText(){return theme()==2?android.graphics.Color.WHITE:0xff14283a;}
...
Photo : ").append(e.photoPath.isEmpty()?"MISSING":"READY");TextView d=label(sb.toString());d.setTextColor(primaryText());box.addView(d);sv.addView(box);
    ...
    Button primary(String s){Button b=btn(s);b.setTextColor(android.graphics.Color.WHITE);b.setBackground(round(0xff1769aa,14));return b;}
